...
pub async fn incr(pool: &MobcPool, key: &str) -> Result<bool> {
    let mut con = get_con(&pool).await?;
    let value = con.incr(key, 1).await.map_err(MobcError::RedisCMDError)?;

    Ok(value)
}

pub async fn decr(pool: &MobcPool, key: &str) -> Result<bool> {
    let mut con = get_con(&pool).await?;
    let value = con.incr(key, -1).await.map_err(MobcError::RedisCMDError)?;
...
pub async fn lpop(pool: &MobcPool, key: &str) -> Result<String> {
    let mut con = get_con(&pool).await?;
    let value = con.lpop(key).await.map_err(MobcError::RedisCMDError)?;

    Ok(value)
}

pub async fn llen(pool: &MobcPool, key: &str) -> Result<u32> {
    let mut con = get_con(&pool).await?;
    let value = con.llen(key).await.map_err(MobcError::RedisCMDError)?;
